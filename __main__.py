  ...
  def _Console():#built-in function,for console.
    ...
    while True:
      ...
      if permissionlvl>=1 and g=="run"or permissionlvl>=1 and g=="start":
        ...
        Kernel.Cout.Msg("use mod:"+str(usemod)+" | mod amounts:"+str(modamount))
...
class Permission:#permission level class.
  ...
  def Warn(id):#built-in function, issue a warning to given id.
    if not Permission.IsValid(id):
      ...
      Kernel.ErrChk(1,"Current user does not exist.")
      return 1
    if str(Kernel.GetVar("userid"))==str(id):
  ...
  def SetGroup(id,level):#built-in function, set current permission level.
    ...
    Kernel.Cout.Msg("Changed user ID %s's permission level to '%s'."%(id,level))
    return id,level
  @staticmethod
  ...
  def Unban(id):#built-in function, unban an user by userid.
    if not Permission.IsValid(id):
      ...
      Kernel.ErrChk(1,"Current user does not exist.")
      return 1
    if Permission.IsBanned(id) is False:
